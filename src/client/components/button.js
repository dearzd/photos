      ...
      return(
        <div id={id} className={classNames.join(' ')}>
          <label>
            ...
            <input type="file" onChange={onChange} multiple={multiple ? 'multiple' : null} accept={accept.join(',')} style={{'display': 'none'}}/>
          </label>
        </div>
      );
    } else {
      return (
        <button id={id} className={classNames.join(' ')} onClick={onClick} disabled={disabled} autoFocus={autoFocus}>
          {icon ? <Icon name={icon} /> : null}
          {text}
        </button>
      );

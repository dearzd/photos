    ...
    return (
      <Title title={'Photos - ' + currentAlbum.name}>
        <div>
          ...
          <div id="album-detail" className={classNames.join(' ')} style={{opacity: !computedWidth ? 0 : null}} ref={c => this.container = c}>
            ...
            <div className="album-photos">
              {
                currentAlbum.images.map((imgInfo, imgIndex) => {
                  return <PhotoView key={imgInfo.name}
            ...
            { ~currentIndex ? <PhotoMaxView albumId={albumId} images={currentAlbum.images} currentIndex={currentIndex} onExit={this.outMaxView} /> : null }
          </div>
        </div>
      </Title>
    );
